...
if [ ! -z "$TEST_FILE_KONG" ]
then
    echo $TEST_FILE_KONG | tee -a test_kong.jmx
...
if [ ! -z "$TEST_FILE_KRAKEND" ]
then
    echo $TEST_FILE_KRAKEND | tee -a test_krakend.jmx
...
if [ ! -z "$TEST_FILE_TYK" ]
then
    echo $TEST_FILE_TYK | tee -a test_tyk.jmx
...
"http://127.0.0.1:5000/configurations/$CONFIG_ID/tests/$TEST_ID" # TODO: CHANGE IP
