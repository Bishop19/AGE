...
if [ ! -z "$TEST_FILE_KONG" ]; then
    ...
    RESULTS_KONG=$(sed -z -E "s/Dev\.\s/Dev.||/g" <<< $RESULTS_KONG)
    RESULTS_KONG=$(sed -z -r "s/([0-9])\s([a-zA-Z])/\1||\2/g" <<< $RESULTS_KONG)
else
    echo "NO KONG CONFIG"
fi
echo "ENDING KONG"
...
if [ ! -z "$TEST_FILE_KRAKEND" ]; then
    ...
    RESULTS_KRAKEND=$(sed -z -E "s/Dev\.\s/Dev.||/g" <<< $RESULTS_KRAKEND)
    RESULTS_KRAKEND=$(sed -z -r "s/([0-9])\s([a-zA-Z])/\1||\2/g" <<< $RESULTS_KRAKEND)
else
